...
class AuthRepository {
  ...
  bool _isFirebaseOwner(String? email) {
    const adminEmails = {'gopim', 'admin'};
    if (email == null) return false;
    return adminEmails.any((admin) => email.toLowerCase().contains(admin.toLowerCase()));
  }
  ...
  Future<bool> sendOtpMsg91(String phone) async {
    ...
    try {
      ...
      debugPrint('[AuthRepository] MSG91 SDK returned non-success response. Falling back to local mock.');
      await _db.collection('otp_codes').doc('otp_$cleanedPhone').set({
        'phone': phone,
        'code': '123456',
        'expiresAt': Timestamp.fromDate(DateTime.now().add(const Duration(minutes: 5))),
      ...
      debugPrint('[AuthRepository] Error calling MSG91 sendOTP SDK: $e. Falling back to local mock.');
      await _db.collection('otp_codes').doc('otp_$cleanedPhone').set({
        'phone': phone,
        'code': '123456',
        'expiresAt': Timestamp.fromDate(DateTime.now().add(const Duration(minutes: 5))),
